...
set -x
/usr/local/bin/ginkgo "${ginkgo_args[@]}" /usr/local/bin/e2e.test -- --disable-log-dump --repo-root=/kubernetes --provider="${E2E_PROVIDER}" --report-dir="${RESULTS_DIR}" --kubeconfig="${KUBECONFIG}" --allowed-not-ready-nodes="${ALLOWED_NOT_READY_NODES:-0}" | tee "${RESULTS_DIR}"/e2e.log &
set +x
